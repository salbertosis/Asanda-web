...
import { comparePerformance, median } from './performance-regression.mjs';
...
const LAB_METRIC_TOLERANCES = { LCP: 100 };
...
try {
  ...
  for (let index = 0; index < SAMPLE_COUNT; index += 1) {
    ...
    samples.push(await page.evaluate(() => ({ CLS: globalThis.__asandaWebVitals?.CLS ?? null, LCP: globalThis.__asandaWebVitals?.LCP ?? null })));
    await page.close();
  }
  const result = { source: 'browser lab Web Vitals median', url: new URL('/', baseUrl).href, sampleCount: SAMPLE_COUNT, samples, metrics: { CLS: median(samples.map(({ CLS }) => CLS)), LCP: median(samples.map(({ LCP }) => LCP)) }, unavailableFieldMetrics: ['INP'] };
  if (outputPath) await writeFile(outputPath, JSON.stringify(result, null, 2) + '\n');
  console.log(JSON.stringify(result));
  if (baselinePath) {
    const comparison = comparePerformance(result, JSON.parse(await readFile(baselinePath, 'utf8')), { metrics: LAB_METRIC_TOLERANCES });
