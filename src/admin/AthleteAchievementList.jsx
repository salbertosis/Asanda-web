...
const recordAvailable = (child) => child.recordStatus === 'published' && child.recordPublishedAt && Date.parse(child.recordPublishedAt) <= Date.now();
const childLabel = (child, type) => {
  ...
  if (type === 'state_record') return `${event} · ${recordAvailable(child) ? 'Récord oficial publicado' : 'Récord oficial no disponible'}`;
...
const canPublish = (item) => item.children.length > 0 && item.children.every((child) => child.eventDefinitionId && child.eventActive && (item.type !== 'state_record' || recordAvailable(child)));
